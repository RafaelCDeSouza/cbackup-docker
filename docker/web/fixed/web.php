...
// Injects ?token=TOKEN query param as Authorization: Bearer TOKEN directly into
// Yii's HeaderCollection so HttpBearerAuth can authenticate Java daemon API calls.
// Must use BootstrapInterface because getallheaders() / $_SERVER injection is too late.
class ApiTokenBootstrap extends \yii\base\BaseObject implements \yii\base\BootstrapInterface {
    public function bootstrap($app) {
        if (!empty($_GET['token'])) {
            $headers = $app->getRequest()->getHeaders();
            if (!$headers->has('Authorization')) {
                $headers->set('Authorization', 'Bearer ' . $_GET['token']);
            }
        }
    }
}
...
$config = [
    ...
    'bootstrap'      => ['log', 'app\helpers\ConfigHelper', 'apiToken'],
	'aliases'        => [
